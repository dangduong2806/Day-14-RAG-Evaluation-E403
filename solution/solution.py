...
# Yếu điểm:
# Phụ thuộc vào câu hỏi (Query): Câu hỏi thường rất ngắn và thiếu các 
# keyword quan trọng có trong câu trả lời mong đợi.
# Không phạt độ dài (No Length Penalty): Một chunk rất dài (ví dụ 500 từ) có 
# cơ hội trùng nhiều từ của query hơn một chunk ngắn, cô đọng (ví dụ 20 từ), 
# dù chunk ngắn có độ tập trung thông tin cao hơn.
...
# Hướng cải tiến 2 (Expected-based Rerank - Chỉ dùng khi Evaluation Offline): 
# So khớp trực tiếp các chunk với expected_answer (câu trả lời chuẩn của chuyên gia) 
# thay vì dùng query.
def rerank_by_expected(contexts: list[str], expected: str) -> list[str]:
    expected_tokens = _tokenize(expected)
    
    def get_expected_score(chunk: str) -> int:
        return len(expected_tokens & _tokenize(chunk))
        
    return sorted(contexts, key=get_expected_score, reverse=True)
